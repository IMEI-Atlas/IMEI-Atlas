...
- New: Partial TAC input (1-8 digits) to generate complete IMEIs.
...
# Config & constants
# -------------------------
AUTHOR_NAME: str = " telegram https://t.me/VHSPC ﺐﻴﻐﻟﺍ ﺮﻬﻈﺑ ﻲﻟ ﻮﻋﺪﺗ نﺍ ﻞﻤﻌﻟﺍ اﺬﻫ مﺍﺪﺨﺘﺳﺍ ﻦﻤﺛ"
...
DISCLAIMER = (
    ...
    "Use only on devices you own and have permission to modify.ﻂﻘﻓ ﺔﻳﺭﺎﺒﺘﺧﻻﺍﻭ ﺔﻴﻤﻴﻠﻌﺘﻟﺍ ضﺍﺮﻏﻸﻟ ﺎًﻳﺮﺼﺣ ﺔﻤﻤﺼُﻣ ةﺍﺩﻷﺍ هﺬﻫ :ﺔﻴﻟﻭﺆﺴﻤﻟﺍ ءﻼﺧﺇ"
)
...
DEVICE_DATABASE: List[Dict[str, Any]] = [
    ...
    {"tac": ["86167907","86638307"], "name": "ZTE", "model": "G5 ULTRA", "type": DeviceType.ROUTER},
    ...
    {"tac": ["01602600","01634700"], "name": "NETGEAR", "model": "M6 Pro MR6400/MR6450/MR6550/MR6150", "type": DeviceType.ROUTER},
    ...
    {"tac": "86968607", "name": "Huawei", "model": "Pro 5 E6888-982", "type": DeviceType.ROUTER},
    {"tac": "86335904", "name": "Mikrotik", "model": "hAP ax lite LTE6", "type": DeviceType.ROUTER},
    {"tac": "35666210", "name": "Mikrotik", "model": "R11E-LTE6", "type": DeviceType.ROUTER},
    ...
    {"tac": ["86510304","86406705"], "name": "Huawei", "model": "5G Outdoor CPE Max N5368X", "type": DeviceType.ROUTER},
    ...
    {"tac": "86762605", "name": "KAER", "model": "5g cpe KL501", "type": DeviceType.ROUTER},
...
class IMEIGenerator:
    ...
    def generate_imei_from_partial_tac(self, partial_tac: str) -> str:
        """Generate IMEI from partial TAC (1-8 digits) by randomly completing it to 8 digits"""
        if not (isinstance(partial_tac, str) and partial_tac.isdigit() and 1 <= len(partial_tac) <= 8):
            raise ValueError("Partial TAC must be 1-8 digits")
        
        # Complete TAC to 8 digits with random numbers
        if len(partial_tac) < 8:
            missing_digits = 8 - len(partial_tac)
            partial_tac += ''.join(str(self.rng.randint(0, 9)) for _ in range(missing_digits))
        
        return self.generate_imei_from_tac(partial_tac)
...
def main(imeis_per_device: int = DEFAULT_IMEIS_PER_DEVICE,
    ...
    while True:
        ...
        partial_tac_index = len(filtered_devices) + 3  # New option
        check_imei_index = len(filtered_devices) + 4
        luhn_index = len(filtered_devices) + 5
        all_export_index = len(filtered_devices) + 6
        search_index = len(filtered_devices) + 7
        help_index = len(filtered_devices) + 8
        ...
        print(apply_color(f"{partial_tac_index:2d}. Generate IMEI with partial TAC (1-8 digits)", Colors.CYAN, use_color))  # New option
        ...
        # Partial TAC option
        if idx == len(filtered_devices) + 2:
            partial_tac_input = input(apply_color("Enter partial TAC (1-8 digits): ", Colors.YELLOW, use_color)).strip()
            if not partial_tac_input.isdigit() or not (1 <= len(partial_tac_input) <= 8):
                print(apply_color("Invalid input. Must be 1-8 digits.", Colors.RED, use_color))
                pause_for_user()
                continue
            
            try:
                # Generate multiple IMEIs with the same partial TAC pattern
                imeis_list = [imei_generator.generate_imei_from_partial_tac(partial_tac_input) for _ in range(imeis_per_device)]
                temp_group = {"tacs": [f"Partial:{partial_tac_input}"], "name": f"Partial TAC: {partial_tac_input}", "imeis": {f"Partial:{partial_tac_input}": imeis_list}}
                should_quit = device_menu_view(temp_group, imei_generator, use_color)
                if should_quit:
                    return
            except Exception as e:
                print(apply_color(f"Error generating IMEIs: {e}", Colors.RED, use_color))
                pause_for_user()
            continue
